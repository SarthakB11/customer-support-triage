...
- Vector Search returned `[]` for every query while the Index node reported `recordsIndexed: 5`. Cause: the node has a `certainty` input (default **0.85**) that is not in the docs' parameter table and not visible unless you open the node panel; with Gemini embeddings an email-vs-FAQ match sits well under that. Setting `certainty: 0.5` fixed it. Also the Vector Store page keeps showing `Records: 0` after two successful index runs, so the UI gives no signal that data is there.
